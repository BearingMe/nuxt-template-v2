...
const el = ref<InstanceType<typeof Popover>>();

defineExpose({
  toggle: (event: Event, target: EventTarget | null) => el.value?.toggle(event, target),
  show: (event: Event, target: EventTarget | null) => el.value?.show(event, target),
  hide: () => el.value?.hide(),
});
</script>
